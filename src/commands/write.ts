...
const referenceArgument = Argument.string('reference').pipe(
  Argument.withDescription('A review-comment:ID, issue-comment:ID, or thread:ID reference'),
);
...
const emitMutation = (agent: boolean, json: boolean, value: unknown): void => {
  emit(toMode(agent, json), value, () => {
    printMutation(value);
  });
};
...
export const commentCommand = Command.make(
  ...
  ({ agent, bodyFile, json, pr, repo, stdin }) =>
    Effect.gen(function* commentCommandGen() {
      ...
      yield* Effect.sync(() => {
        emitMutation(agent, json, created);
      });
    }),
...
export const editCommand = Command.make(
  ...
  ({ agent, bodyFile, json, pr, reference, repo, stdin }) =>
    Effect.gen(function* editCommandGen() {
      ...
      const body = yield* readMarkdown({ bodyFile, stdin });
      const updated = yield* editComment(target, selection, body);
      yield* Effect.sync(() => {
        emitMutation(agent, json, updated);
      });
    }),
...
export const reviewCommand = Command.make(
  ...
  ({ agent, bodyFile, event, json, pr, repo, stdin }) =>
    Effect.gen(function* reviewCommandGen() {
      ...
      const review = yield* submitReview(target, githubEvent, body);
      yield* Effect.sync(() => {
        emitMutation(agent, json, review);
      });
    }),
  ...
  Command.make(
    ...
    ({ agent, json, pr, reference, repo }) =>
      Effect.gen(function* threadMutationCommandGen() {
        ...
        const result =
          name === 'resolve'
            ? yield* resolveThread(target, thread)
            : yield* unresolveThread(target, thread);
        yield* Effect.sync(() => {
          emitMutation(agent, json, result);
        });
      }),
  ).pipe(
